<template>
  <section id='section5' class="pt-5 pb-5 structure">
    <!-- Titre de la section -->
    <h1 class="titleSection pb-3 pb-md-5 color-text">Contact</h1>
    <!-- Fin du titre de la section -->
    <div class="container">
      <!-- DEBUT DU FORMULAIRE -->
      <form id="formulaire" action="#">
        <div class="row pt-5 pb-5">
          <div class="offset-1 col-10 offset-md-1 col-md-5 ">
            <div class="form-group">
              <label class="col-form-label color-text" for="lname">Nom</label>
              <input type="text" name="lname" id="lname" class="form-control" required title="Entrez votre nom.">
            </div>
            <div class="form-group">
              <label class="col-form-label color-text" for="fname">Prénom</label>
              <input type="text" name="fname" id="fname" class="form-control" required title="Entrez votre prénom.">
            </div>
            <div class="form-group">
              <label class="col-form-label color-text" for="email">Email</label>
              <input type="email" name="email" id="email" class="form-control" required
                     title="Entrez votre adresse email.">
            </div>
            <div class="form-group">
              <label class="col-form-label color-text" for="tel">Téléphone</label>
              <input type="text" name="tel" id="tel" class="form-control" required
                     title="Entrez votre numéro de téléphone.">
            </div>
          </div>
          <div class="offset-1 col-10 offset-md-0 col-md-5 ">
            <div class="form-group">
              <label for="message" class="col-form-label color-text">Message</label>
              <textarea name="message" id="message" class="form-control" required title="Entrez votre message."
                        style="height: 320px;"></textarea>
            </div>
          </div>
        </div>
        <!-- Bouton qui valide le formulaire -->
        <div class="row justify-center align-items">
          <div class="offset-4 col-4 mb-5">
            <button type="submit" class="bouton-valider btn btn-primary w-100 color-text">Valider</button>
          </div>
        </div>
      </form>
      <!-- FIN DU FORMULAIRE -->
      <!-- OMBRES -->
      <div class="drop drop-1"></div>
      <div class="drop drop-2"></div>
      <div class="drop drop-3"></div>
      <div class="drop drop-4"></div>
      <div class="drop drop-5"></div>
    </div>
  </section>
</template>

<script>
export default {
  name: "ContactForm"
}
</script>

<style scoped>

:root {
  --white: #ffffff;
  --twhite: rgb(255, 255, 255, .3);
  --dark-green: #00a4c1;
  --light-green: #00adc1;
}

.titleSection {
  font-size: 2em;
}

h1 {
  font-family: 'Parisienne', Verdana, Helvetica, serif;
  text-align: center;
}

/******************        SECTION 5  ( CONTACT )   *******************/

#section5 {
  background: linear-gradient(45deg, var(--light-green), var(--dark-green));
  color: var(--white);
}

#section5 .container {
  position: relative;
  width: 80%;
  margin: 0 auto;
  z-index: 1;
}

#formulaire {
  border-radius: 1em;
  background-color: var(--twhite);
  border-left: 1px solid var(--twhite);
  border-top: 1px solid var(--twhite);
  box-shadow: 20px 20px 40px -6px rgba(0, 0, 0, .2);
  backdrop-filter: blur(10px);
  -webkit-backdrop-filter: blur(10px);
  -moz-backdrop-filter: blur(10px);
}

#message {
  /* Le resize none permet d'enlever le curseur qui agrandit l'espace de texte*/
  resize: none;
}

#section5 label, button {
  font-family: 'Philisopher', Verdana, Helvetica, sans-serif;
  opacity: .8;
  font-size: 1.1em;
}

#section5 input, #message {
  background: transparent;
  border: none;
  border-left: 1px solid var(--twhite);
  border-top: 1px solid var(--twhite);
  border-radius: 10px;
  backdrop-filter: blur(5px);
  -webkit-backdrop-filter: blur(5px);
  -moz-backdrop-filter: blur(5px);
  box-shadow: 4px 4px 60px rgba(0, 0, 0, .2);
}

.bouton-valider {
  background: #26c481;
  border-color: var(--light-green);
  box-shadow: 4px 4px 60px rgba(0, 0, 0, .2);
  padding: 10px;
  border: none;
  transition: all 1s;
}

.drop {
  position: absolute;
  background: var(--twhite);
  backdrop-filter: blur(5px);
  -webkit-backdrop-filter: blur(5px);
  -moz-backdrop-filter: blur(5px);
  border-left: 1px solid var(--twhite);
  border-top: 1px solid var(--twhite);
  border-radius: 10px;
  box-shadow: 10px 10px 60px -8px rgba(0, 0, 0, .2);
}

.color-text {
  color: white;
}

.drop-1 {
  height: 90px;
  width: 90px;
  top: -20px;
  left: -30px;
  z-index: -1;
}

.drop-2 {
  height: 120px;
  width: 120px;
  bottom: -20px;
  right: -40px;
  z-index: 1;
}

.drop-3 {
  height: 24em;
  width: 15em;
  bottom: 30%;
  right: 20%;
  z-index: -1;
}

</style>

